...
counts_for_deseq = pd.read_csv("outputs_TG_full/counts_filtered_for_deseq.csv", index_col=0)
...
metadata["treatment"] = pd.Categorical(metadata["treatment"], categories=["Untreated", "BIM+BAK", "BIM-PF"])
metadata["sex"] = pd.Categorical(metadata["sex"], categories=["Female", "Male"])
metadata["genotype"] = pd.Categorical(metadata["genotype"], categories=["Healthy", "Mutant"])
...
for sex in ["Female", "Male"]:
    for genotype in ["Healthy", "Mutant"]:
        ...
        dds_sub = DeseqDataSet(
            counts=sub_counts, metadata=sub_meta, design="~treatment",
        ...
        for treat in ["BIM+BAK", "BIM-PF"]:
            ...
            stat.results_df.to_csv(f"outputs_TG_full/DE_{label}.csv")
...
summary_df.to_csv("outputs_TG_full/DE_summary_by_subgroup.csv", index=False)
print("\n--- RESUME PARTIE A ---")
print(summary_df.to_string(index=False))

# ============================================================
# PARTIE B : MODELE FACTORIEL COMPLET (reference = Untreated)
# ============================================================
print("\n\n########## PARTIE B: modele factoriel complet (60 echantillons) ##########")
dds_full = DeseqDataSet(
    counts=counts_for_deseq, metadata=metadata,
...
for col in interaction_cols:
    ...
    stat.results_df.to_csv(f"outputs_TG_full/DE_interaction_{safe_name}.csv")
    n_sig = (stat.results_df["padj"] < 0.05).sum()
    print(f"{col}: {n_sig} genes significatifs (padj<0.05)")
    interaction_summary.append({"interaction_term": col, "n_significant": n_sig})

pd.DataFrame(interaction_summary).to_csv("outputs_TG_full/interaction_summary.csv", index=False)
print("\n=== TERMINE ===")
