...
os.makedirs("outputs_full/gsea_per_subgroup_BIMBAK", exist_ok=True)
...
subgroups = {
    "Female_Healthy": "outputs_full/DE_Female_Healthy_BIMplusBAK_vs_Untreated.csv",
    "Female_Mutant":  "outputs_full/DE_Female_Mutant_BIMplusBAK_vs_Untreated.csv",
    "Male_Healthy":   "outputs_full/DE_Male_Healthy_BIMplusBAK_vs_Untreated.csv",
    "Male_Mutant":    "outputs_full/DE_Male_Mutant_BIMplusBAK_vs_Untreated.csv",
}
    ...
    for db_name, gene_sets in [("GO_BP", go_sets), ("KEGG", kegg_sets)]:
        try:
            pre_res = gp.prerank(
                ...
                outdir=f"outputs_full/gsea_per_subgroup_BIMBAK/{label}_{db_name}",
                seed=42,
                min_size=5,
                max_size=1000,
            )
            res = pre_res.res2d
            res.to_csv(f"outputs_full/gsea_per_subgroup_BIMBAK/{label}_{db_name}_results.csv", sep=";")
...
summary_df.to_csv("outputs_full/gsea_per_subgroup_BIMBAK/summary_top_pathways.csv", sep=";", index=False)
print("\n\n=== RESUME FINAL ===")
